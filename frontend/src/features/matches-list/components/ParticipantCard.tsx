...
export function ParticipantCard({participant}: ParticipantCardProps) {
    return (
        <Card className={participant.teamId === '100' ? 'bg-cyan-700': 'bg-red-400/70'}>
            <CardContent>
                <>
                    <div className="flex justify-between items-center">
                        <h3 className="font-bold">{participant.summoner.gameName}</h3>
                        <Avatar>
                            <AvatarImage className="rounded-none" src={`https://wiki.leagueoflegends.com/en-us/images/thumb/${(participant.teamPosition[0].toUpperCase()+participant.teamPosition.slice(1).toLowerCase()).replace('Utility', 'Support')}_icon.png/120px-${(participant.teamPosition[0].toUpperCase()+participant.teamPosition.slice(1).toLowerCase()).replace('Utility', 'Support')}_icon.png`}/>
                        </Avatar>
                    </div>
                    ...
                    <div className="flex gap-4">
                        ...
                        <div className="shadow-2xl shadow-black w-fit">
                            <ChampionIcon icon={participant.championName} level={participant.champLevel} />
                        </div>
                        <div>
