...
export async function createMatches(request: RiotMatchesRequest) {
    ...
    for (const match of matches) {
        ...
        for (const participant of matchData.info.participants) {
            ...
            try {
                await createSummoner({ gameName: participant.riotIdGameName, tagLine: participant.riotIdTagline });

                await prisma.participant.upsert({
                    where: { participantId: { summonerId: participant.puuid, matchId: match } },
                    update: {},
                    create: {
